...
from .configuration_t5la import T5LaConfig, T5LaModuleConfig
...
class T5LaRMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        # Llama does x.to(float16) * w whilst T5La is (x * w).to(float16)
...
class T5LaAttention(nn.Module):
    ...
    def __init__(self, config: T5LaConfig, layer_idx: int):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
        self.head_dim = getattr(config, "head_dim", config.hidden_size // config.num_attention_heads)
