...
"""
Convert T5LAX checkpoint to PyTorch

Steps:
- Install gsutil according to https://cloud.google.com/storage/docs/gsutil_install
- Get a T5LAX checkpoint at https://github.com/google-research/t5lax/blob/main/docs/models.md#t5la-11-checkpoints Example:
    `gsutil -m cp -r gs://t5la-data/pretrained_models/t5lax/t5la_1_1_small $HOME/`
- Create or download a corresponding config for the downloaded model. E.g. for T5LA v1.1 small, you can use
    https://huggingface.co/google/t5la-v1_1-small/blob/main/config.json
- Convert:
    ```
    python3 convert_t5lax_checkpoint_to_pytorch.py --t5lax_checkpoint_path=$HOME/t5la_1_1_small --config_file=config.json\
      --pytorch_dump_path=$HOME/t5la_1_1_small_pt
    ```
"""
...
from transformers import T5LAConfig, T5LAEncoderModel, T5LAForConditionalGeneration
...
def t5lax_attention_lookup(params, i, prefix, layer_name="attention"):
...
def t5lax_mlp_lookup(params, i, prefix, split_mlp_wi=False):
...
def t5lax_layer_norm_lookup(params, i, prefix, layer_name):
    """Returns the layer norm param of a layer."""
    return params[f"{prefix}/layers_{i}/{layer_name}/scale"]


def convert_t5lax_to_pytorch(variables: dict, *, num_layers: int, num_decoder_layers: int, is_encoder_only: bool):
    """Converts the parameters from T5LAX-Flax to Transformers-PyTorch."""
    ...
    for i in range(num_layers):
        ...
        layer_norm = t5lax_layer_norm_lookup(old, i, "encoder", "pre_attention_layer_norm")
...
def load_t5lax_weights_in_t5la(model, config, t5lax_checkpoint_path, is_encoder_only):
    """Replaces the params in model witht the T5LAX converted params."""
    variables = checkpoints.load_t5lax_checkpoint(t5lax_checkpoint_path)
    converted = convert_t5lax_to_pytorch(
        variables,
...
def convert_t5lax_checkpoint_to_pytorch(
    t5lax_checkpoint_path, config_file, pytorch_dump_path, is_encoder_only: bool = False
):
    """Loads the config and model, converts the T5LAX checkpoint, and saves a PyTorch checkpoint."""
    # Initialise PyTorch model
    config = T5LAConfig.from_json_file(config_file)
    print(f"Building PyTorch model from configuration: {config}")
    # Non-v1.1 checkpoints could also use T5LAModel, but this works for all.
    # The v1.0 checkpoints will simply have an LM head that is the word embeddings.
    if is_encoder_only:
        model = T5LAEncoderModel(config)
    else:
        model = T5LAForConditionalGeneration(config)

    # Load weights from tf checkpoint
    load_t5lax_weights_in_t5la(model, config, t5lax_checkpoint_path, is_encoder_only)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Converts a native T5LAX checkpoint into a PyTorch checkpoint.")
    # Required parameters
    parser.add_argument(
        "--t5lax_checkpoint_path", default=None, type=str, required=True, help="Path to the T5LAX checkpoint."
    )
    parser.add_argument(
        "--config_file",
        default=None,
        type=str,
        required=True,
        help="The config json file corresponding to the pre-trained T5LA model.\nThis specifies the model architecture.",
    )
    parser.add_argument(
    ...
    convert_t5lax_checkpoint_to_pytorch(
        args.t5lax_checkpoint_path, args.config_file, args.pytorch_dump_path, args.is_encoder_only
    )
