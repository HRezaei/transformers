...
class T5LaConfig(PretrainedConfig):
    ...
    >>> model = T5LaModel(t5la_config)
    ```
    Configuration objects inherit from [PretrainedConfig] and can be used to control the model outputs. Read the
    documentation from [PretrainedConfig] for more information.
    Args:
        encoder (`Union[T5LaModuleConfig, dict]`, optional, *optional*):
            Configuration for the encoder.
        decoder (`Union[T5LaModuleConfig, dict]`, optional, *optional*):
    ...
    def __init__(
        self,
        encoder: Optional[Union[T5LaModuleConfig, dict[Any, Any]]] = None,
        decoder: Optional[Union[T5LaModuleConfig, dict[Any, Any]]] = None,
        ...
        **kwargs,
        ...
        if isinstance(encoder, dict):
            encoder = T5LaModuleConfig(**encoder)
        elif encoder is None:
            encoder = T5LaModuleConfig()
        else:
            assert isinstance(encoder, T5LaModuleConfig), f"{type(encoder)} is not supported."

        if isinstance(decoder, dict):
            decoder = T5LaModuleConfig(**decoder)
        elif decoder is None:
            decoder = encoder
        else:
            assert isinstance(decoder, T5LaModuleConfig), f"{type(decoder)} is not supported."

        encoder = T5LaModuleConfig(**encoder.to_dict())
        decoder = T5LaModuleConfig(**decoder.to_dict())
...
class T5LaRMSNorm(Gemma2RMSNorm):
    pass


class T5LaMLP(Gemma2MLP):
...
class T5LaRotaryEmbedding(Gemma2RotaryEmbedding):
    def __init__(self, config, device=None):
        super().__init__(config, device)


class T5LaSelfAttention(Gemma2Attention):
    def __init__(self, config: T5LaModuleConfig, layer_idx: int):
        super().__init__(config, layer_idx)
        # Requied by flash attention: encoder selfattention is non-causal
        self.is_causal = config.is_decoder


class T5LaCrossAttention(Gemma2Attention):
    def __init__(self, config: T5LaModuleConfig, layer_idx: int):
        super().__init__(config, layer_idx)
...
class T5LaEncoderLayer(GradientCheckpointingLayer):
    ...
    def __init__(self, config, layer_idx: int):
        ...
        self.self_attn = T5LaSelfAttention(
            config=config,
            layer_idx=layer_idx,
        )
        self.pre_self_attn_layernorm = T5LaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_self_attn_layernorm = T5LaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)

        self.mlp = T5LaMLP(config)
        self.pre_feedforward_layernorm = T5LaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_feedforward_layernorm = T5LaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class T5LaDecoderLayer(T5LaEncoderLayer):
    ...
    def __init__(self, config, layer_idx: int):
        super().__init__(config, layer_idx)
        self.cross_attn = T5LaCrossAttention(config=config, layer_idx=layer_idx)
        self.pre_cross_attn_layernorm = T5LaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_cross_attn_layernorm = T5LaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
@auto_docstring
class T5LaPreTrainedModel(Gemma2PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, T5LaRMSNorm):
        ...
        elif isinstance(module, T5LaLMHead):
...
class T5LaEncoder(T5LaPreTrainedModel):
    _can_record_outputs = {
        ...
        "hidden_states": T5LaEncoderLayer,
    }
    ...
    def __init__(self, config):
        ...
        self.norm = T5LaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = T5LaRotaryEmbedding(config=config)
        self.gradient_checkpointing = False

        self.layers = nn.ModuleList(
            [T5LaEncoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5LaDecoder(T5LaEncoder):
    _can_record_outputs = {
        ...
        "cross_attentions": OutputRecorder(T5LaCrossAttention, index=1),
        "hidden_states": T5LaDecoderLayer,
    }

    def __init__(self, config):
        super().__init__(config)
        self.layers = nn.ModuleList(
            [T5LaDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class T5LaModel(T5LaPreTrainedModel):
    def __init__(self, config: T5LaConfig):
        ...
        if not config.is_encoder_decoder:
            raise ValueError("T5LaModel only support encoder-decoder modeling. Use `T5LaEncoderModel` instead.")

        self.encoder = T5LaEncoder(config.encoder)
        self.decoder = T5LaDecoder(config.decoder)
...
class T5LaEncoderModel(T5LaPreTrainedModel):
    def __init__(self, config: T5LaConfig):
        ...
        if config.is_encoder_decoder:
            raise ValueError("T5LaEncoderModel only supports encoder-only model. Use `T5LaModel` instead.")
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if self.training and self.config._attn_implementation != "eager":
            msg = (
                "It is strongly recommended to train T5La models with the `eager` attention implementation "
...
class T5LaForSequenceClassification(T5LaPreTrainedModel):
    def __init__(self, config: T5LaConfig, is_encoder_decoder: Optional[bool] = None):
        ...
        if config.is_encoder_decoder:
            ...
            self.model = T5LaEncoderModel(config)
        ...
        self.score = T5LaClassificationHead(hidden_size, self.num_labels, classifier_dropout)
        self.post_init()
