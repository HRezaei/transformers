...
"""
Fine-tuning the library models for seq2seq language modeling (GPT, GPT-2, CTRL, ...) on a text file or a dataset.

Here is the full list of checkpoints on the hub that can be fine-tuned by this script:
https://huggingface.co/models?filter=text-generation
"""
# You can also adapt this script on your own seq2seq language modeling task. Pointers for this are left as comments.
...
@dataclass
class MemorizationArguments:
    """
    Arguments for measuring how quickly the model learns the first k samples without altering training.
    """

    measure_k: int = field(
        default=0,
        metadata={"help": "If >0, monitor learning on the first k train samples during the normal training loop."},
    )
    measure_target_loss: float = field(
        default=0.01,
...
def main():
    ...
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, TrainingArguments, MemorizationArguments))
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        # If we pass only one argument to the script and it's the path to a json file,
        # let's parse it to get our arguments.
        model_args, data_args, training_args, mem_args = parser.parse_json_file(json_file=os.path.abspath(sys.argv[1]))
    else:
        model_args, data_args, training_args, mem_args = parser.parse_args_into_dataclasses()
    ...
    def group_texts_for_lookahead_efficient(examples):
        ...
        labels_dict = {
            "lookahead_targets": [
                t[i + block_size : i + block_size + lookahead_size] for i in range(0, total_length, sequence_length)
            ]
            for k, t in concatenated_examples.items()
            if k == "input_ids"
        }
        ...
        for k, t in concatenated_examples.items():
            ...
            if k == "input_ids":
                ...
                for i in range(0, total_length, block_size):
                    samples.append(
                        [
                            t[i + look_head_index : i + look_head_index + block_size]
