...
from transformers.trainer_utils import EvalLoopOutput, EvalPrediction, SaveStrategy, denumpify_detensorize, has_length
...
class LookaheadTrainer(Trainer):
        ...
        if self.state.is_world_process_zero:
            ...
            for k, v in logs.items():
                if k.startswith("eval_"):
                    ...
                    metrics_to_log[k] = v
            super().log(metrics_to_log, start_time)
        else:
    ...
    def compute_loss(
        ...
        lookahead_loss = outputs["lookahead_loss"].detach().cpu().item()
        if not hasattr(self, "lookahead_losses"):
            self.lookahead_losses = {
                "train": [],
                "eval": [],
            }
