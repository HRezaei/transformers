...
from la_trainer import LookaheadTrainer
...
class ModelArguments:
    ...
    random_baseline: bool = field(
        default=False,
        metadata={"help": "If set to True, the model will be loaded with random weights. Used for evaluating baseline."},
    )
...
def main():
    ...
    if os.environ.get("WANDB_PROJECT") == "LLM-Eval":
        wandb.login()
        config_for_wandb = {
        ...
        wandb.init(
                ...
                settings=wandb.Settings(_service_wait=300, code_dir=".", init_timeout=300),
                dir="~/scratch",
                id=wandb_run_id,
                resume=wandb_resume
        )
    ...
    if lookahead_size and lookahead_size > 0:
        if lookahead_type == "la":
            ...
            for head in model.la_heads.heads:
