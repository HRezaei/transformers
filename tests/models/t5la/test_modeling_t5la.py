...
"""Testing suite for the PyTorch T5La model."""
...
from transformers import T5LaConfig, T5LaModuleConfig, is_torch_available
...
class T5LaModelTester:
    ...
    module_config_class = T5LaModuleConfig

    if is_torch_available():
        model_class = T5LaModel
        for_causal_lm_class = T5LaForConditionalGeneration
        for_sequence_class = T5LaForSequenceClassification
        for_token_class = T5LaForTokenClassification
...
class T5LaModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    _torch_compile_train_cls = T5LaForConditionalGeneration if is_torch_available() else None
    # `t5la` will give warning or raise error if it is not `eager` during training.
    ...
    # Based on tests.models.gemma.test_modeling_gemma.GemmaModelTest.test_Gemma_sequence_classification_model with Gemma -> T5La (Add is_encoder_decoder option)
    def test_T5La_sequence_classification_model(self):
    ...
    # Based on tests.models.gemma.test_modeling_gemma.GemmaModelTest.test_Gemma_sequence_classification_model_for_single_label with Gemma -> T5La (Add is_encoder_decoder option)
    def test_T5La_sequence_classification_model_for_single_label(self):
        config, input_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.num_labels = 3
    ...
    # Based on tests.models.gemma.test_modeling_gemma.GemmaModelTest.test_Gemma_sequence_classification_model_for_multi_label with Gemma -> T5La (Add is_encoder_decoder option)
    def test_T5La_sequence_classification_model_for_multi_label(self):
        config, input_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.num_labels = 3
    ...
    # Based on tests.models.gemma.test_modeling_gemma.GemmaModelTest.test_Gemma_token_classification_model with Gemma -> T5La (Add is_encoder_decoder option)
    def test_T5La_token_classification_model(self):
        config, input_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.num_labels = 3
        input_ids = input_dict["input_ids"]
        attention_mask = input_ids.ne(1).to(torch_device)
    ...
    @unittest.skip("T5La eager/FA2 attention outputs are expected to be different")
    ...
    @unittest.skip("Mismatch issue doesn't exist in T5La.")
...
class T5LaEncoderOnlyModelTester:
    ...
    def create_and_check_with_token_classification_head(
        ...
        labels = torch.tensor([1] * self.seq_length * self.batch_size, dtype=torch.long, device=torch_device)
        model = T5LaForTokenClassification(config=config, is_encoder_decoder=False).to(torch_device).eval()
        outputs = model(
            input_ids=input_ids,
            labels=labels,
            attention_mask=attention_mask,
...
@require_torch
class T5LaEncoderOnlyModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    @unittest.skip("No loss in the output of T5LaEncoderModel")
