...
class T5LaModelTester:
    ...
    def check_prepare_lm_labels_via_shift_left(
        ...
        model = T5LaForConditionalGeneration(config=config)
    ...
    def create_and_check_model(
        ...
        decoder_output = result.decoder_last_hidden_state
    ...
    def create_and_check_with_lm_head(
        ...
        self.parent.assertEqual(len(outputs), 7)
        self.parent.assertEqual(outputs["logits"].size(), (self.batch_size, self.decoder_seq_length, self.vocab_size))
        self.parent.assertEqual(outputs["loss"].size(), ())
        self.parent.assertEqual(
            outputs["lookahead_logits"].size(),
            (self.batch_size, self.lookahead_size, self.decoder_seq_length, self.vocab_size),
        )
    ...
    def create_and_check_decoder_model_past(
        ...
        model = T5LaForConditionalGeneration(config=config).get_decoder().to(torch_device).eval()
        # first forward pass
    ...
    def create_and_check_decoder_model_attention_mask_past(
        ...
        model = T5LaForConditionalGeneration(config=config).get_decoder()
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_encoder_decoder_shared_weights(
        ...
        for model_class in [T5LaForConditionalGeneration]:
            torch.manual_seed(0)
...
class T5LaModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (T5LaForConditionalGeneration,) if is_torch_available() else ()
    ...
    all_parallelizable_model_classes = (T5LaForConditionalGeneration,) if is_torch_available() else ()
    fx_compatible = True
    test_pruning = False
...
class T5LaModelFp16Tests(unittest.TestCase):
    def test_fp16_fp32_conversion(self):
        ...
        with unittest.mock.patch("builtins.__import__", side_effect=import_accelerate_mock):
            ...
            model = T5LaForConditionalGeneration.from_pretrained("google-t5/t5-small", torch_dtype=torch.float16)
            ...
            model = T5LaForConditionalGeneration.from_pretrained("google-t5/t5-small", torch_dtype=torch.bfloat16)
...
class T5LaModelIntegrationTests(unittest.TestCase):
    ...
    @cached_property
    def model(self):
        return T5LaForConditionalGeneration.from_pretrained("google-t5/t5-base").to(torch_device)
    ...
    def test_contrastive_search_t5(self):
        ...
        t5_model = T5LaForConditionalGeneration.from_pretrained("flax-community/t5-base-cnn-dm").to(torch_device)
    ...
    def test_export_decoder(self):
        """Test exporting T5La decoder with static cache to torch export format."""
        if not is_torch_greater_or_equal_than_2_4:
            self.skipTest("This test requires torch >= 2.4 to run.")

        from transformers import AutoModelForSeq2SeqLM, T5LaForConditionalGeneration
        ...
        self.assertIsInstance(full_model, T5LaForConditionalGeneration)
        decoder_model = (
        ...
        # For T5La-small, hidden size is 512
    ...
    def test_export_t5_summarization(self):
        """Test composing exported T5La encoder and decoder for summarization."""
        if not is_torch_greater_or_equal_than_2_4:
            self.skipTest("This test requires torch >= 2.4 to run.")

        from transformers import AutoModelForSeq2SeqLM, AutoTokenizer, T5LaForConditionalGeneration
        ...
        original_model = T5LaForConditionalGeneration.from_pretrained(model_id).eval()
        with torch.no_grad():
...
class TestAsymmetricT5La(unittest.TestCase):
    def build_model_and_check_forward_pass(self, **kwargs):
        ...
        assert outputs["lookahead_logits"].size() == (
            tester.batch_size,
            tester.lookahead_size,
