# coding=utf-8
# Copyright 2025 Google T5LA Authors and HuggingFace Inc. team.
#
...
class T5LAModelTester:
    ...
    def get_large_model_config(self):
        return T5LAConfig.from_pretrained("google-t5la/t5la-base")
    ...
    def get_pipeline_config(self):
        return T5LAConfig(
            vocab_size=166,  # t5la forces 100 extra tokens
    ...
    def check_resize_embeddings_t5la_v1_1(
        self,
        config,
    ):
...
class T5LAModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = (T5LAForConditionalGeneration,) if is_torch_available() else ()
    ...
    # The small T5LA model needs higher percentages for CPU/MP tests
    model_split_percents = [0.5, 0.8, 0.9]

    def setUp(self):
        self.model_tester = T5LAModelTester(self)
        self.config_tester = ConfigTester(self, config_class=T5LAConfig, d_model=37)

    # overwrite because T5LA doesn't accept position ids as input and expects `decoder_input_ids`
    ...
    def test_v1_1_resize_embeddings(self):
        ...
        self.model_tester.check_resize_embeddings_t5la_v1_1(config)

    @slow
    def test_model_from_pretrained(self):
        model_name = "hrezaei/T5LA"
        model = T5LAModel.from_pretrained(model_name)
        self.assertIsNotNone(model)

    @unittest.skip(reason="Test has a segmentation fault on torch 1.8.0")
    def test_export_to_onnx(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        model = T5LAModel(config_and_inputs[0]).to(torch_device)
        with tempfile.TemporaryDirectory() as tmpdirname:
            torch.onnx.export(
                model,
                (config_and_inputs[1], config_and_inputs[3], config_and_inputs[2]),
                f"{tmpdirname}/t5la_test.onnx",
                export_params=True,
                opset_version=9,
    ...
    def test_generate_with_head_masking(self):
        ...
        model = T5LAForConditionalGeneration(config).eval()
        model.to(torch_device)
        ...
        for attn_name, (name, mask) in zip(attention_names, head_masking.items()):
            ...
            # Explicitly pass decoder_head_mask as it is required from T5LA model when head_mask specified
            if name == "head_mask":
...
@require_tokenizers
@slow
class T5LAModelFp16Tests(unittest.TestCase):
    def test_fp16_fp32_conversion(self):
        ...
        with unittest.mock.patch("builtins.__import__", side_effect=import_accelerate_mock):
            ...
            model = T5LAForConditionalGeneration.from_pretrained("hrezaei/T5LA", torch_dtype=torch.float16)
            ...
            model = T5LAForConditionalGeneration.from_pretrained("hrezaei/T5LA", torch_dtype=torch.bfloat16)
            self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wo.weight.dtype == torch.bfloat16)
            self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wi.weight.dtype == torch.bfloat16)

        # Load using `accelerate` in bf16
        model = T5LAForConditionalGeneration.from_pretrained(
            "hrezaei/T5LA", torch_dtype=torch.bfloat16, device_map="auto"
        )
        self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wo.weight.dtype == torch.bfloat16)
        self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wi.weight.dtype == torch.bfloat16)
        ...
        # Load using `accelerate` in bf16
        model = T5LAForConditionalGeneration.from_pretrained(
            "hrezaei/T5LA", torch_dtype=torch.bfloat16, low_cpu_mem_usage=True
        )
        self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wo.weight.dtype == torch.bfloat16)
        self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wi.weight.dtype == torch.bfloat16)
        ...
        # Load without using `accelerate`
        model = T5LAForConditionalGeneration.from_pretrained(
            "hrezaei/T5LA", torch_dtype=torch.float16, low_cpu_mem_usage=True
        )
        self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wo.weight.dtype == torch.float32)
        self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wi.weight.dtype == torch.float16)
        ...
        model = T5LAForConditionalGeneration.from_pretrained(
            "hrezaei/T5LA", torch_dtype=torch.float16, device_map="auto"
        )
        self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wo.weight.dtype == torch.float32)
        self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wi.weight.dtype == torch.float16)
...
@require_sentencepiece
@require_tokenizers
class T5LAModelIntegrationTests(unittest.TestCase):
    @cached_property
    def model(self):
        return T5LAForConditionalGeneration.from_pretrained("google-t5la/t5la-base").to(torch_device)
    ...
    def test_small_integration_test(self):
        """
        For comparision run:
        >>> import t5la  # pip install t5la==0.7.1
        >>> from t5la.data.sentencepiece_vocabulary import SentencePieceVocabulary

        >>> path_to_mtf_small_t5la_checkpoint = '<fill_in>'
        >>> path_to_mtf_small_spm_model_path = '<fill_in>'
        >>> t5la_model = t5la.models.MtfModel(model_dir=path_to_mtf_small_t5la_checkpoint, batch_size=1, tpu=None)
        >>> vocab = SentencePieceVocabulary(path_to_mtf_small_spm_model_path, extra_ids=100)
        >>> score = t5la_model.score(inputs=["Hello there"], targets=["Hi I am"], vocabulary=vocab)
        """
    ...
    def test_small_v1_1_integration_test(self):
        ...
        >>> path_to_mtf_small_t5la_v1_1_checkpoint = '<fill_in>'
        >>> path_to_mtf_small_spm_model_path = '<fill_in>'
        >>> t5la_model = t5la.models.MtfModel(model_dir=path_to_mtf_small_t5la_v1_1_checkpoint, batch_size=1, tpu=None)
        >>> vocab = SentencePieceVocabulary(path_to_mtf_small_spm_model_path, extra_ids=100)
        >>> score = t5la_model.score(inputs=["Hello there"], targets=["Hi I am"], vocabulary=vocab)
        """
    ...
    @slow
    def test_small_byt5la_integration_test(self):
        """
        For comparision run:
        >>> import t5la  # pip install t5la==0.9.1

        >>> path_to_byt5la_small_checkpoint = '<fill_in>'
        >>> t5la_model = t5la.models.MtfModel(model_dir=path_to_tf_checkpoint, batch_size=1, tpu=None)
        >>> vocab = t5la.data.ByteVocabulary()
        >>> score = t5la_model.score(inputs=["Hello there"], targets=["Hi I am"], vocabulary=vocab)
        """
    ...
    def test_translation_en_to_fr(self):
        model = self.model  # google-t5la/t5la-base
        tok = self.tokenizer
    ...
    @slow
    def test_contrastive_search_t5la(self):
        article = (
            " New York (CNN)When Liana Barrientos was 23 years old, she got married in Westchester County, New York. A"
            " year later, she got married again in Westchester County, but to a different man and without divorcing"
            " her first husband.  Only 18 days after that marriage, she got hitched yet again. Then, Barrientos"
        ...
        t5la_tokenizer = AutoTokenizer.from_pretrained("flax-community/t5la-base-cnn-dm")
        t5la_model = T5LAForConditionalGeneration.from_pretrained("flax-community/t5la-base-cnn-dm").to(torch_device)
        input_ids = t5la_tokenizer(
            article, add_special_tokens=False, truncation=True, max_length=512, return_tensors="pt"
        ).input_ids.to(torch_device)

        outputs = t5la_model.generate(input_ids, penalty_alpha=0.5, top_k=5, max_length=64)
        generated_text = t5la_tokenizer.batch_decode(outputs, skip_special_tokens=True)
...
@require_torch
class TestAsymmetricT5LA(unittest.TestCase):
    def build_model_and_check_forward_pass(self, **kwargs):
        tester = T5LAModelTester(self, **kwargs)
    ...
    def test_small_decoder(self):
        # num_hidden_layers is passed to T5LAConfig as num_layers
